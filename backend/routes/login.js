...
{  
  ...
  mysqlClientInstance.exec( 'SELECT st_code, st_password FROM thesis_st_user WHERE st_code=? AND st_password=?', [ formData.username, password_md5 ], function ( err, rows, fields )
  {
    if ( err )
    {
    ...
    {
      console.log( 'SELECT st_code, st_password FROM thesis_st_user WHERE st_code=' + formData.username + ' AND st_password=' + password_md5 + ' success' );
      ...
      {
        response.cookie( 'userId', rows[ 0 ].st_code, {
          maxAge: 24 * 60 * 60 * 1000,//cookie 生命值
          path: '/',
          httpOnly: true
        } );
